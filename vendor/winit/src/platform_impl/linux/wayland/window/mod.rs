...
impl Window {
    ...
    pub fn data_drag_context(&self) -> Option<(*mut std::ffi::c_void, u32)> {
        self.window_state.lock().unwrap().data_drag_context()
    }

    pub fn start_file_drag(&self, uri_list: Vec<u8>) -> Result<(), ExternalError> {
        let manager = self.data_device_manager.as_ref().ok_or_else(|| {
            ExternalError::NotSupported(NotSupportedError::new())
        })?;
        let state = self.window_state.lock().unwrap();
        let pointer = state.pointers.iter().filter_map(Weak::upgrade).next().ok_or_else(|| {
            ExternalError::NotSupported(NotSupportedError::new())
        })?;
        let pointer_data = pointer.pointer().winit_data();
        let serial = pointer_data.latest_button_serial();
        if serial == 0 {
            return Err(ExternalError::NotSupported(NotSupportedError::new()));
        }
        let device = manager.get_data_device(&self.queue_handle, pointer_data.seat());
