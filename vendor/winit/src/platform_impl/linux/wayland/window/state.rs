...
impl WindowState {
    ...
    /// Raw seat proxy and latest compositor-issued button serial for a native
    /// data-device drag. The values are borrowed from winit's active pointer
    /// and are valid only during the initiating input dispatch.
    pub(crate) fn data_drag_context(&self) -> Option<(*mut std::ffi::c_void, u32)> {
        self.pointers.iter().filter_map(Weak::upgrade).next().map(|pointer| {
            let data = pointer.pointer().winit_data();
            (data.seat().id().as_ptr().cast(), data.latest_button_serial())
