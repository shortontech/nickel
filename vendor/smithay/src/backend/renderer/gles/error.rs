...
pub enum GlesError {
    ...
    SyncInterrupted,
    /// A frame could not export completion without a synchronous fallback.
    #[error("Unable to export a frame completion fence without blocking")]
    SyncExportFailed,
}
...
impl From<GlesError> for SwapBuffersError {
    ...
    fn from(err: GlesError) -> SwapBuffersError {
        match err {
            ...
            | x @ GlesError::SyncInterrupted
            | x @ GlesError::SyncExportFailed => SwapBuffersError::TemporaryFailure(Box::new(x)),
        }
    }
