...
For HTTPS, verify the supplied certificate's SHA-256 fingerprint with the desktop owner over a trusted channel before installing it. A self-signed host certificate or a dedicated issuing CA can be used. The adapter trusts only that explicit certificate, disables built-in public roots, and still checks the endpoint hostname. Use a hostname present in the certificate. There is no insecure certificate override. HTTP is accepted only for a literal loopback address; `http://localhost` is rejected so DNS or hosts-file resolution cannot redirect plaintext credentials; remote URLs must use HTTPS. Redirects and environment HTTP proxies are disabled. URLs cannot contain credentials, queries, or fragments. Stderr reports a generic failure without request, response, credential, or URL details.
