...
fn main() {
    eprintln!("post-window-layout supports Windows only");
}
...
fn main() -> windows::core::Result<()> {
    ...
    let wrapper = usize::from_str_radix(
        arguments
            .next()
            .expect("usage: post-window-layout WRAPPER HWND")
    ...
    unsafe { PostMessageW(Some(shell), 0x806f, WPARAM(wrapper), LPARAM(hwnd as isize))? };
    println!(
        "phase=post-window-layout shell={:?} wrapper={wrapper:#x} hwnd={hwnd:#x}",
        shell.0
    );
    Ok(())
}
