...
mod internal_shell_placement_tests {
    ...
    #[cfg(any())]
    pub(super) fn receive_shared_x11_text_requests(
        display: &str,
        headers_ready: std::sync::mpsc::Sender<()>,
        start_first: std::sync::mpsc::Receiver<()>,
        first_done: std::sync::mpsc::Sender<()>,
        start_second: std::sync::mpsc::Receiver<()>,
        ...
        let properties = [atom(b"NICKEL_SHARED_UTF8")?, atom(b"NICKEL_SHARED_TEXT")?];
        let incr = atom(b"INCR")?;
        for index in 0..2 {
            connection
                .convert_selection(
        ...
        let deadline = Instant::now() + Duration::from_secs(15);
        let mut notified = [false; 2];
        let mut active_index = 0;
        let mut bytes = [Vec::new(), Vec::new()];
        loop {
            if Instant::now() >= deadline {
                return Err("shared X11 text requests timed out".into());
            }
            let Some(event) = connection.poll_for_event().map_err(|e| e.to_string())? else {
                std::thread::sleep(Duration::from_millis(1));
                continue;
            ...
            match event {
                Event::SelectionNotify(event) if event.requestor == requestor => {
                    let Some(index) = properties
                        .iter()
                        .position(|property| *property == event.property)
                    else {
                        return Err("shared request was rejected".into());
                    };
                    let reply = connection
                        .get_property(
                            false,
                    ...
                    if reply.type_ != incr {
                        return Err(format!(
                            "shared request {index} did not enter INCR (type {}, bytes {})",
                            reply.type_,
                            reply.value.len()
                        ));
                    }
                    notified[index] = true;
                    if notified == [true, true] {
                        headers_ready.send(()).map_err(|e| e.to_string())?;
                        start_first
                            .recv_timeout(Duration::from_secs(5))
                            .map_err(|e| e.to_string())?;
                ...
                Event::PropertyNotify(event)
                    if event.window == requestor
                        && event.state == Property::NEW_VALUE
                        && properties.contains(&event.atom) =>
                {
                    let index = properties
                        .iter()
                        .position(|property| *property == event.atom)
                        .unwrap();
                    ...
                    if reply.value.is_empty() {
                        if active_index == 0 {
                            first_done.send(()).map_err(|e| e.to_string())?;
                            start_second
                                .recv_timeout(Duration::from_secs(5))
                                .map_err(|e| e.to_string())?;
                            active_index = 1;
                            ...
                            if pending.value.is_empty() {
                                return Err("second shared transfer had no pending chunk".into());
                            }
                            bytes[1].extend_from_slice(&pending.value);
                            connection.flush().map_err(|e| e.to_string())?;
                        } else {
                            return Ok((
                                std::mem::take(&mut bytes[0]),
                                std::mem::take(&mut bytes[1]),
                            ));
                        }
                    } else {
                        bytes[index].extend_from_slice(&reply.value);
                        connection.flush().map_err(|e| e.to_string())?;
                    }
                }
                _ => {}
...
mod protocol_tests {
    ...
    fn native_xwayland_input_only_requestor_receives_complete_incremental_png() {
...

        {
            ...
            let (observation_connection, screen) =
                smithay::reexports::x11rb::connect(Some(&display)).unwrap();
            let observation_connection = Arc::new(observation_connection);
            let root = &observation_connection.setup().roots[screen];
            let requestor = observation_connection.generate_id().unwrap();
            ...
            .unwrap();
            drop(first);
            let retained = observation_connection
            ...
            drop(second);
            let restored = observation_connection
                .get_window_attributes(requestor)
                .unwrap()
                .reply()
                .unwrap()
        ...
        #[cfg(any())]
        {
            let shared_text_bytes = vec![b's'; 65_537];
            let shared_text = String::from_utf8(shared_text_bytes.clone()).unwrap();
            session.publish_native_text_selection(shared_text).unwrap();
            let shared_display = display.clone();
            let (headers_tx, headers_rx) = std::sync::mpsc::channel();
            let (start_first_tx, start_first_rx) = std::sync::mpsc::channel();
            ...
            std::thread::spawn(move || {
                let result =
                    super::internal_shell_placement_tests::receive_shared_x11_text_requests(
                        &shared_display,
                        headers_tx,
                        start_first_rx,
                        first_done_tx,
                ...
                let _ = shared_tx.send(result);
            });
            let shared_deadline = Instant::now() + Duration::from_secs(15);
            while headers_rx.try_recv().is_err() {
                if let Ok(result) = shared_rx.try_recv() {
                    panic!("shared requestor ended before both headers: {result:?}");
                }
                event_loop
                    .dispatch(Duration::from_millis(10), &mut session)
                    .unwrap();
                assert!(Instant::now() < shared_deadline);
            }
            assert_eq!(
                session
                    .xwm
                    ...
                    .unwrap()
                    .1
                    .selection_requestor_observation_count(),
                2
            );
            start_first_tx.send(()).unwrap();
            while first_done_rx.try_recv().is_err() {
                if let Ok(result) = shared_rx.try_recv() {
                    panic!("shared requestor ended before first transfer: {result:?}");
                }
                event_loop
                    .dispatch(Duration::from_millis(10), &mut session)
                    .unwrap();
                ...
                != 1
            {
                event_loop
                    ...
                    .unwrap();
                assert!(
                    Instant::now() < shared_deadline,
                    "shared observer count remained {}",
                    session
                        .xwm
                        .as_ref()
                        .unwrap()
            ...
            let (shared_utf8, shared_plain) = loop {
                event_loop
                    .dispatch(Duration::from_millis(10), &mut session)
                    .unwrap();
                match shared_rx.try_recv() {
                    Ok(result) => break result.unwrap(),
                    Err(std::sync::mpsc::TryRecvError::Empty) => {}
                    Err(std::sync::mpsc::TryRecvError::Disconnected) => {
                        panic!("shared requestor exited without a result")
                    }
                }
                assert!(Instant::now() < shared_deadline);
            };
            assert_eq!(shared_utf8, shared_text_bytes);
            assert_eq!(shared_plain, shared_text_bytes);
            assert_eq!(
                session
                    .xwm
                    .as_ref()
        ...
        for reject_transfer in [true, false] {
            ...
            std::thread::spawn(move || {
                let result =
                    super::internal_shell_placement_tests::serve_x11_pending_clipboard_fixture(
                        &fixture_display,
                        reject_transfer,
                        ready_tx,
                        requested_tx,
