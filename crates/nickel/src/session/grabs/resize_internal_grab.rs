...
impl PointerGrab<NickelSession> for ResizeInternalSurfaceGrab {
    ...
    fn unset(&mut self, data: &mut NickelSession) {
        ...
        if data.finish_internal_resize(self.surface, compensate) {
    ...
    fn motion(
        ...
        placement.geometry = (
            proposal.x,
            proposal.y,
            proposal.width as u32,
            proposal.height as u32,
        );
        if data.apply_internal_resize(self.surface, placement) {
            data.request_output_redraw();
        }
    }
