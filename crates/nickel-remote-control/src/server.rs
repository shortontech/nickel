...
impl McpHandler {
    ...
    #[tool(
        description = "Run a typed production diagnostic action with a Full Control & Debug Nickel lease. repaint requests a complete redraw; refresh_scene reconciles scene/output membership and removes dead scene entries; refresh_application_inventory asks the bounded production application catalog worker for a fresh scan; refresh_platform_status asks one allowlisted production worker for connectivity, audio, peripherals, maintenance, or default-association state. Refresh results carry their own observation timestamps, preparation duration, partial state, and reconciliation outcome. These acknowledgements do not confirm presentation. identify_output shows the existing output number on one exact live output for up to 3 seconds, with cancellation/revocation cleanup; it never supplies custom text or confirms presentation. start_frame_trace enables a bounded frame CPU dispatch trace for 1..=60 seconds (256 records, one active trace globally); stop_frame_trace stops your lease's trace. Read records in diagnostic_snapshot.frame_trace. The trace category identifies the nested or DRM backend; timings do not confirm GPU completion or presentation."
    )]
