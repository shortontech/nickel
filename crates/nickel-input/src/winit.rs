...
pub fn physical_key(key: WinitPhysicalKey) -> PhysicalKey {
    ...
    let portable = match key {
        ...
        // Preserve portable command/navigation identities for both native desktop
        // scancode conversion and external winit hosts; do not alias keypad Enter.
...
mod tests {
    use super::*;
    #[test]
    fn desktop_command_keys_keep_portable_physical_identities() {
        for (native, portable) in [
            ...
            assert_eq!(
                physical_key(WinitPhysicalKey::Code(native)),
                PhysicalKey::Code(portable)
            );
        }
    }
