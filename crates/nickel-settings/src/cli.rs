...
fn parse_screen(value: &str) -> Result<SettingsPage, String> {
    match value {
        ...
        "printers" | "storage" | "printers-storage" => Ok(SettingsPage::PrintersStorage),
        "security" | "security-updates" => Ok(SettingsPage::Security),
        ...
        _ => Err(format!(
            "unknown screen '{value}'; expected display, nickel-bar, appearance, network, bluetooth, bluetooth-pair, printers-storage, security, default-apps, optional-features, keyboard-shortcuts, or about"
        )),
    }
}
