...
| Launch under an output lease | Verified Wayland/X11 launch ancestry now drives output placement before scene insertion, under the original lease and output incarnation. Nested native success/cancellation/replacement tests passed. The integrated Wayland movement harness also launches a real catalog executable, observes its mapped window confined to the requested virtual output and uses the same lease for focus and capture. | Complete broker/daemon attribution and broader physical, overload, deadline and cancellation acceptance. |
| Full diagnostic coverage | The compositor snapshot explicitly reports unavailable domains through a closed server-owned vocabulary. Warning/error history exposes fixed severity and subsystem categories, a numeric static-callsite code and source line while omitting messages, fields, targets and source paths. Existing production desktop transitions carry an exhaustive fixed event category. Ordinary shell transients use production visibility and protection evidence; typed shell actions, effect ordering, every typed platform refresh, and bounded external accessibility are integrated. Pointer diagnostics correlate hosted UI hits with a live bounded semantic tree generation/node ordinal or a fixed Nickel frame role. Linux native dispatch accounting retains fixed cumulative CPU duration/count/max/generation/timestamp fields after production output dispatch returns, and its shared host-texture cache owner exposes aggregate entry/byte/peak/activity plus allocation/upload counts. Neither record contains outputs, cache keys, paths, pixels, text, or native handles, and neither queries or waits for GPU completion. GPU completion timing, driver allocation size, external client resources, and per-surface shared-cache attribution remain explicit gaps. The Windows owner returns a generation-bearing, freshly revalidated snapshot containing its protected-filtered scoped windows/outputs, focus, remote held-input state, operation/admission/lease metrics, trace lifecycle and payload-free structured warning/error records. Windows shell-surface records combine monotonic native incarnations, winit geometry/output/scale/focus/redraw evidence, and LiveShell visibility/protection/scene generations; lock, Codex, trusted, hidden, disconnected, or incomplete records are omitted and the 128-record bound reports truncation. Windows has no compositor-hosted ordinary application clients: native Nickel tools remain in the scoped native-window inventory, Winit chrome remains in the shell-surface inventory, and protected Codex surfaces enter neither, so `internal_applications` is an authoritative empty domain rather than an unavailable collector. Their production presenters add successful native presentation generations, failure counters and last per-surface frame bytes without attributing the shared rasterizer or caches. Generation-correlated shell image-cache counts and retained bytes are exposed without keys, paths, pixels, titles, or application identities; preview totals are filtered through the exact protected and resource-scoped native-window projection. Windows DWM preview diagnostics expose a generation and update failures only while every current source belongs to that same projection; pixels, titles and native identities are absent. DWM pixel readback and shared renderer/presenter-cache attribution remain explicitly unavailable. The Windows Codex diagnostic is projected from the production launcher state and includes only support, installation, enablement, health, and configuration generation. Full-debug repaint and scene refresh queue redraw through the production winit owner, while scene refresh first reconciles a fresh bounded native inventory. Application-inventory refresh coalesces onto the existing production worker. All five Windows platform-refresh domains execute against production providers on a single-flight bounded worker before the presentation owner, retain typed generation/timestamps/coarse status, and publish payload-free completion events. Maintenance PowerShell starts suspended and enters a kill-on-close job before execution, with bounded output and continuous deadline and permit checks. These actions record a payload-free requested effect and do not claim presentation or reconciliation completion. Windows event polling/subscriptions use the same full-debug owner boundary and retain remote keyboard/drag ownership plus ordinary shell visibility and keyboard-focus transitions. Shell events carry only the native surface incarnation and fixed role; current winit visibility/focus and LiveShell visibility/protection must agree, stale queued events are omitted, protected recipients become a payload-free focus clear, and unchanged focus is coalesced. | Complete GPU completion timing and driver allocation evidence where a production API exists, remaining shared renderer attribution, remaining safe actions, remaining production-effect event and trace categories, and native Windows validation. |
...
`windows_per_surface_renderer_cache_attribution`; GPU completion timing, driver allocations, and
DWM pixel readback remain unavailable.
...
to revoke runtime authority before queued work after a lock transition. Local
lease approval and resumption now remain suspended until fresh resource validation
and trusted indication succeed, but native Windows execution is not yet proven.
The physical-key hook now latches an atomic cancellation epoch immediately.
...
Windows remote launch requests now enter the production owner queue and revalidate
the exact catalog generation, application ID, verified application identity,
...
remain open. Windows now starts each production PowerShell helper suspended,
assigns it to a kill-on-close job before script execution, bounds retained output,
and polls the shared diagnostic deadline and live permit. The coarse maintenance
projection is therefore available through the Windows single-flight diagnostic
worker without forwarding output or provider errors. Cross-link and Wine execution
cover the containment path; native Windows acceptance remains required. No source
files were added; the exact inventory remains 344.
...
On Windows, connectivity, audio, peripheral, maintenance and default-association refreshes now
use those same typed outcomes and freshness rules. Collection runs before the
presentation owner on one single-flight worker with a two-second response bound;
revoked full-debug authority prevents publication. Timeout leaves admission held
until the production provider actually exits, preventing overlapping abandoned
queries. The projection contains counts and availability only, and does not carry
SSIDs, device or printer labels, filesystem paths, association handlers, provider
errors or PowerShell output. Maintenance PowerShell processes start suspended and
cannot execute until assigned to a kill-on-close job; the helper kills that job on
the shared deadline or live-permit revocation and waits for the child while draining
only bounded output.
...
same production surface incarnation, current native visibility/focus, and LiveShell
visibility/protection evidence used by the coherent snapshot. A queued visibility
event is discarded if those observations no longer match. Lock, Codex, trusted,
protected, disconnected and unknown surfaces expose no identity; focus entering
one of those recipients can only produce the generic cleared state. Focus state is
coalesced, and the shared 128-record history still retains no input payloads,
geometry, output names, titles, text, paths or client identity. Every snapshot and
...
executes all 59 categories through success, error and cancelled outcomes, requires
...
test requires exact label coverage for all 58 published tools plus the bounded
event-subscription path, so a new tool cannot silently omit its metric category.
Admission rejection counters use only five fixed categories: global concurrency,
global rate, client concurrency, client rate and client capacity. They remain
available before lease approval and carry no identity, peer, resource or payload
labels.

The current `nickel-remote-control` library suite has 152 tests and its host all-target/all-feature Clippy
