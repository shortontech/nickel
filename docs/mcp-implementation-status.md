...
On Windows, connectivity, audio, peripheral and default-association refreshes now
...
errors or PowerShell output. Maintenance remains explicitly unavailable because
its current production provider starts PowerShell without cancellable child-job
containment; timing out that process would allow it to outlive revocation.
