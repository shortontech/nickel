...
| Platform-neutral UI | active-widget identity, shared pointer/keyboard/controller activation prefix, handled/fallback disposition | pass: 400 passed, 2 ignored | pass | not applicable | none for enumerated headless transitions |
...
Exact current-tip results below were recorded at `3eaf7cf`. Older focused evidence remains listed
...
`nickel-session-protocol` 56/0, `nickel-ui` 400/0 with 2 ignored, and
