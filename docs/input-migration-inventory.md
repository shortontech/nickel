...
| Accessibility semantic invocation | trusted/native accessibility adapters -> semantic UI action | resource/semantic node identity and action execution identity | stale/revoked nodes and modal-excluded direct actions fail closed; bounded observation and dispatch cancellation | migrated boundary |
...
| Internal compositor-surface move | input -> internal move grab | `Enforced`; surface generation bound; maximize supersedes the move and clears its baseline/grab before geometry changes | migrated |
