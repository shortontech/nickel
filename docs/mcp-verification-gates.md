...
| 11 Delayed subsystem snapshots | Renderer/worker snapshots expose own observations; allowlisted platform and application-catalog refreshes retain their own generation, observation interval, five-second freshness and shared worker state; unavailable domains remain explicit. | Controlled delayed GPU work and remaining platform queries proving timestamp/staleness semantics and unaffected input/presentation. |
