...
| 3 Sustained actions in each scope | Linux owner adapters implement window/application/output/full-session operations. The Windows owner now admits local approval/resume only after fresh exact resource validation and trusted-indicator exposure, routes ordinary input/window-manager requests through generation, scope, process, geometry, lifecycle, permit and secure-desktop revalidation, then obtains separate fresh observed outcomes. Windows authorized-window capture and bounded read-only external window UIA use the same owner identity boundary and final fresh owner revalidation. | Complete Windows external semantic actions and application-wide UIA, plus the live multi-action matrix for every scope on both platforms; execute approval, input, capture and UIA natively on Windows. |
