...
| Activation | Two SCTK native Wayland clients | A focused source's fresh pointer serial produced a token that activated the mapped target. Mapping an untrusted requester did not steal focus; consuming its serial-less token was rejected and the original target remained focused. | Pending |
